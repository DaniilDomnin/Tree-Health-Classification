...
Все категориальные признаки и некоторые числовые признаки, которые по смыслу являются категориальными, будем обратывать с помощью эмбеддинг слоев. Остальную архитектуру будем строить по принципу линейный слой -> нормализация -> функция активации -> дропаут.
